...
public static class Solution
{
    ...
    private static string[] FindPath(
        (string id, string name, double latitude, double longitude)[] stops,
        ...
        int end)
    {
        ...
        stopQueue.Enqueue(start, double.MaxValue);
        dist[start] = 0;
        while (stopQueue.Count > 0)
        {
            ...
            for (int i = 0; map[currentStop][i].HasValue; i++)
            {
                if (dist[currentStop] + map[currentStop][i].Value.distance < dist[map[currentStop][i].Value.destination])
                {
                    ...
                    stopQueue.Enqueue(map[currentStop][i].Value.destination, dist[map[currentStop][i].Value.destination]);
                }
            }
        }
    ...
    public static string[] Solve(string[] input)
    {
        ...
        var stops = new (string id, string name, double latitude, double longitude)[stopCount];
        ...
        var (_, _, endLatitude, endLongitude) = stops[endPointId];
        ...
        for (int i = 0; i < routeCount; i++)
        {
            ...
            map[from][routesPerStop[from]++] =
                (to, GetDistance(stops[from].latitude, stops[to].latitude, stops[from].longitude, stops[to].longitude)
                    - GetDistance(stops[from].latitude, endLatitude, stops[from].longitude, endLongitude)
                    + GetDistance(stops[to].latitude, endLatitude, stops[to].longitude, endLongitude));
        }
